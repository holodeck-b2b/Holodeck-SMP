...
@Setter
@NoArgsConstructor
public class PgProcInfoFormData {
	...
	/*
	 * Because {@link Identifier} cannot be directly handled by Spring we use the String representation of the 
	 * Identifier to select the Process
	 */
	@NotNull(message = "A process must be selected")
	private String	procID;

	/*
	 * The Identifiers of the Role Participants act in. For display we can use the Set itself, but for submission we
	 * will use a comma separated list of the Role identifier string representations
	 */
	private Set<EmbeddedIdentifier>	roles;	
	private String rolesCSL;

	public PgProcInfoFormData(int pg) {
		this.pgIndex = pg;
		this.procIndex = -1;
	}

	public PgProcInfoFormData(int pg, int p) {
		this.pgIndex = pg;
		this.procIndex = p;
	}

	public PgProcInfoFormData(int pg, int p, ProcessInfoEntity pi) {
		this.pgIndex = pg;
		this.procIndex = p;
		this.procID = IdUtils.toIDString(pi.getProcess().getId());
